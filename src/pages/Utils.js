...
const timeAgo=(date, language) => {
  ...
  let currentTimeUnits=runningTime.time > 1 && runningTime.unit === 4 &&  language === "0" ? "meses" : runningTime.time > 1 ? texts.timeUnits[language][runningTime.unit]+"s": texts.timeUnits[language][runningTime.unit];
...
export const ShareLink = ({language,value,type}) =>{
  ...
  return(
    ...
    <div style={{marginLeft:"0px"}} onClick={()=>copyValue()}>
      {error && 
