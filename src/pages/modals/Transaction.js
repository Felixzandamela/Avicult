  ...
  const handleAlert = (message,stack,notification) =>{
    ...
    if(notification){
      const newNotification = {
        type:notification,
        id:idGenerator(22),
        owner: datas.owner.id,
        amount:150,
        link:`/cabinet/transactions/${type}/view/${datas.id}`,
        seen:false,
    ...
    switch(confirm.status){
            ...
            if(snapDep.exists()){
              ...
              if(/^(Completed|Inprogress)$/i.test(depositStatus)){
                dbCommissions.child(datas.id).update({
                  ...
                  updateBalance(datas.amount, datas.owner.id,"commissionAdded","financialComission");
                }).catch((error)=>{handleAlert(error.message,"error",null)});
              }
            }else{handleAlert(texts.actionNotAllowed[language],"error",null);}
          });
                ...
                snapCom.forEach((snapData)=>{
                  ...
                  if(id && !/^(Completed|Voided)$/i.test(status)){
                    dbCommissions.child(id).update({status:"Completed"}).then(()=>{
                      updateBalance(totalReceivable, owner,"commissionAdded","financialComission");
                    }).catch((error)=>{handleAlert(error.message,"error",null)});
      ...
      case "Rejected":
        colections(type).child(datas.id).update({
          ...
          handleAlert(getSuccesMsg("RejectedSuccessfully"), "success","hasBeenRejected");
        }).catch((error)=>{handleAlert(error.message,"error",null)});
      break;
      case "Voided":
        if(/^(withdrawals|commissions)$/i.test(type)){
          colections(type).child(datas.id).update({status:"Voided"}).then(()=>{
            if(type === "withdrawals"){
              updateBalance(datas.amount, datas.owner.id,"VoidedSuccessfully", "hasVoided");
            }else{
              handleAlert(getSuccesMsg("VoidedSuccessfully"), "success","hasVoided");
            }
          }).catch((error)=>{handleAlert(error.message,"error",null)});
        }
        if(type === "deposits"){
          dbDeposits.child(datas.id).update({
            ...
            dbCommissions.orderByChild("from").equalTo(datas.id).on("value", (snapCom)=>{
              if(snapCom.exists()){
                snapCom.forEach((snapData)=>{
                  let commission = snapData.val();
                  let commissionId = commission.id;
                  if(commissionId){
                    console.log(commissionId);
                    dbCommissions.child(commissionId).update({status:"Voided"}).then(()=>{
                      handleAlert(getSuccesMsg("VoidedSuccessfully"), "success","hasVoided");
                    }).catch((error)=>{handleAlert(error.message,"error",null)});
                  }
                });
              }else{handleAlert(getSuccesMsg("VoidedSuccessfully"), "success","hasVoided");}
            });
          }).catch((error)=>{handleAlert(error.message,"error",null)});
        }
        break;
    ...
    <div className="modal_body">
      ...
      <div className="flex_c_c">
        <div className="m_gateway flex_c_c">
          <img style={{width:"26px",marginRight:"10px"}}src={datas && datas.paymentDetails.gateway.avatar.src} className=""/>
      ...
      {isPaymentInstantly && <div className="m_payments_gateways_wrap">
        <div className="m_payments_gateways">
          ...
          <div className=" pdd5_t"><b>{texts.name[language]}:</b> {datas && datas.paymentDetails.gateway.owner}</div>
          ...
          <div className="flex_b_c">
            <div className=""><b>{texts.amount[language]}</b> {datas && parseFloat(datas.amount).toFixed(2)}</div>
